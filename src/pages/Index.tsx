        ...
        <nav className="flex-1 py-4 px-3 space-y-1">
          {NAV.map((n) => (
            ...
            >
              ...
              {n.label}
              {active === n.id && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-sidebar-primary" />}
            </button>
          ))}
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-border bg-card flex items-center justify-between px-5 md:px-8 sticky top-0 z-10">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Обзор продаж · Сентябрь 2026</p>
          </div>
        ...
        <main className="flex-1 overflow-auto p-5 md:p-8 space-y-6">
          {/* KPI */}
